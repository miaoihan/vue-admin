...
const actions = {
  login ({ commit }, {username, password, callback}) {
    dataService.login(username, password).then(data => {
      ...
      localStorage.setItem('auth', JSON.stringify({token: state.token, id: data.body.profile.id}))
  ...
  reloadAuth ({ dispatch, commit }, callback) {
    let auth = JSON.parse(localStorage.getItem('auth'))
    if (auth !== null && auth !== '') {
      dataService.getAdmin(auth.id).then(data => {
        commit(types.LOGIN, {token: auth.token, profile: data.body})
        if (callback && (callback instanceof Function)) {
          callback()
        }
      })
